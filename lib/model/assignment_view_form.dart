...
class AssignmentViewForm extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (formAction == FormAction.ShowData) {
      ...
      return Scaffold(
        appBar: StyleRegistry.registry().styleWithContext(context).adminFormStyle().constructAppBar(context, formAction == FormAction.UpdateAction ? 'Update AssignmentView' : 'Add AssignmentView'),
    ...
    return BlocBuilder<AssignmentViewFormBloc, AssignmentViewFormState>(builder: (context, state) {
      ...
      if (state is AssignmentViewFormInitialized) {
        ...
        children.add(
                  ...
                  StyleRegistry.registry().styleWithContext(context).adminFormStyle().textFormField(context, 'Document ID', Icons.vpn_key, (formAction == FormAction.UpdateAction), _documentIDController, FieldType.String, validator: (_) => state is DocumentIDAssignmentViewFormError ? state.message : null, hintText: 'null')
          );

        children.add(

                  StyleRegistry.registry().styleWithContext(context).adminFormStyle().textFormField(context, 'description', Icons.text_format, _readOnly(accessState, state), _titleController, FieldType.String, validator: (_) => state is TitleAssignmentViewFormError ? state.message : null, hintText: 'null')
          );

        children.add(

                  StyleRegistry.registry().styleWithContext(context).adminFormStyle().textFormField(context, 'description', Icons.text_format, _readOnly(accessState, state), _descriptionController, FieldType.String, validator: (_) => state is DescriptionAssignmentViewFormError ? state.message : null, hintText: 'null')
          );
