...
class AssignmentFormBloc extends Bloc<AssignmentFormEvent, AssignmentFormState> {
  ...
  Stream<AssignmentFormState> mapEventToState(AssignmentFormEvent event) async* {
    ...
    if (currentState is AssignmentFormUninitialized) {
      ...
      if (event is ChangedAssignmentReporter) {
        if (event.value != null)
          ...
          newValue = new AssignmentModel(
                                 ...
                                 triggeringAssignment: currentState.value.triggeringAssignment,
      ...
      if (event is ChangedAssignmentTriggeringAssignment) {
        if (event.value != null)
          newValue = currentState.value.copyWith(triggeringAssignment: await assignmentRepository(appId: appId).get(event.value));
        else
          newValue = new AssignmentModel(
                                 documentID: currentState.value.documentID,
                                 appId: currentState.value.appId,
