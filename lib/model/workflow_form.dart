...
class WorkflowForm extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (formAction == FormAction.ShowData) {
      ...
      return Scaffold(
        appBar: StyleRegistry.registry().styleWithContext(context).adminFormStyle().constructAppBar(context, formAction == FormAction.UpdateAction ? 'Update Workflow' : 'Add Workflow'),
    ...
    return BlocBuilder<WorkflowFormBloc, WorkflowFormState>(builder: (context, state) {
      ...
      if (state is WorkflowFormInitialized) {
        ...
        children.add(
                  ...
                  StyleRegistry.registry().styleWithContext(context).adminFormStyle().textFormField(context, 'Name', Icons.text_format, _readOnly(accessState, state), _nameController, FieldType.String, validator: (_) => state is NameWorkflowFormError ? state.message : null, hintText: 'null')
          );
        ...
        children.add(
                  ...
                  StyleRegistry.registry().styleWithContext(context).adminFormStyle().textFormField(context, 'Document ID', Icons.vpn_key, (formAction == FormAction.UpdateAction), _documentIDController, FieldType.String, validator: (_) => state is DocumentIDWorkflowFormError ? state.message : null, hintText: 'null')
          );

        children.add(

                  StyleRegistry.registry().styleWithContext(context).adminFormStyle().textFormField(context, 'App Identifier', Icons.text_format, _readOnly(accessState, state), _appIdController, FieldType.String, validator: (_) => state is AppIdWorkflowFormError ? state.message : null, hintText: 'This is the identifier of the app to which this feed belongs')
          );
